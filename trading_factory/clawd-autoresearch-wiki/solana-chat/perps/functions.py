...
JUPITER_QUOTE_BASE = "https://quote-api.jup.ag/v6"  # v6 quote API (no key needed)
JUPITER_TX_BASE = "https://api.jup.ag/tx/v1"        # /submit endpoint
...
def get_jupiter_quote(input_mint: str, output_mint: str, amount: float) -> dict:
    ...
    # Use v2 /build if API key present (advanced path with raw instructions),
    # otherwise fall back to v6 /quote (keyless, quote-only)
    if JUPITER_API_KEY:
        url = (
            f"{JUPITER_BASE}/build?inputMint={in_mint}&outputMint={out_mint}"
            f"&amount={amount_raw}&slippageBps=50&taker=11111111111111111111111111111111"
        )
        r = _fetch(url, headers=_jup_headers())
    else:
        url = f"{JUPITER_QUOTE_BASE}/quote?inputMint={in_mint}&outputMint={out_mint}&amount={amount_raw}&slippageBps=50"
        r = _fetch(url)
    if isinstance(r, dict) and "error" in r:
        return r
    ...
    return {
        ...
        "endpoint": "v2/build" if JUPITER_API_KEY else "v6/quote",
    }
